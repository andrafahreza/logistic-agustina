...
@section('contents')
    ...
    <div class="modal fade" id="tambah" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('simpan-admin') }}" method="POST" id="formTambah">
                    @csrf
    ...
    <div class="modal fade" id="hapus" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('hapus-admin') }}" method="POST" id="formTHapus">
                    @csrf
    ...
    <div class="modal fade" id="nonActive" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('nonactive-admin') }}" method="POST" id="formTnonactive">
                    @csrf
                    <input type="hidden" name="id" id="idNonActive">
                    <div class="modal-header">
                        <h5 class="modal-title">Nonaktifkan admin</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <p>Yakin ingin menonaktifkan admin ini?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
    ...
    <div class="modal fade" id="active" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('active-admin') }}" method="POST" id="formActive">
                    @csrf
                    <input type="hidden" name="id" id="idActive">
                    <div class="modal-header">
                        <h5 class="modal-title">Aktifkan admin</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <p>Yakin ingin mengaktifkan admin ini?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
...
@push("scripts")
    ...
    <script>
        $('#btnTambah').click(function() {
            ...
            $('#formTambah')[0].reset();
            $('#formTambah').attr("action", "{{ route('simpan-admin') }}")
        });

        function edit(id) {
            var url = "{{ route('data-admin') }}" + "/" + id;
            $.ajax({
                type: "get",
                url: url,
                dataType: "JSON",
                success: function(response) {
                    if (response.alert == '1') {
                        ...
                        $('#formTambah')[0].reset();
                        $('#formTambah').attr("action", "{{ route('simpan-admin') }}" + "/" + data.id);
